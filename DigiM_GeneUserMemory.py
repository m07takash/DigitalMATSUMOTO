...
def _merge_persona_lists(existing: list, new: list, max_items: int = 5) -> list:
    """existing(承認/修正済はキープ) と new をラベル単位でマージ。"""
    ...
    for item in new:
        ...
        if lbl in by_label:
            cur = by_label[lbl]
            # 承認/修正済はラベル維持、信頼度のみ寄せて更新
            if cur["status"] in ("approved", "edited"):
    ...
    # deletedはマージ後も保持される（ユーザーが削除したものを再度提案しない用途）
    items = list(by_label.values())
    # status優先 (approved > edited > pending > deleted) → confidence降順
    status_order = {"approved": 0, "edited": 1, "pending": 2, "deleted": 3}
    ...
    return visible[:max_items] + deleted
...
def merge_long_persona(service_id: str, user_id: str, mid_profiles=None) -> dict:
    ...
    for f in _LONG_LIST_FIELDS:
        merged_lists[f] = _merge_persona_lists(existing.get(f) or [], parsed.get(f) or [], max_items=5)
...
def update_long_item_status(service_id: str, user_id: str, field: str, label: str, status: str, new_label: str = "") -> bool:
    ...
    new_label: status="edited" のとき、ラベルを差し替えるテキスト。
    """
    ...
    if status == "edited" and new_label:
