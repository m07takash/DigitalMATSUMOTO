                                            ...
                                            with chat_expander_analytics:
                                                ...
                                                for _gu_rag_name, _gu_usage in sorted(_gu_result_main.items(), key=lambda kv: _ku_order_key(kv[0])):
                                                    ...
                                                    try:
                                                        ...
                                                        # Sensitive-name safety: default is "hover" so an
                                                        # Identity DB (which may hold personal / niche terms)
                                                        # doesn't display its labels during demos.
                                                        #   hover   — nothing shown; mouse-over reveals one label
                                                        #   colored — labels only on colored (used-in-retrieval /
                                                        #             used-in-output) nodes and edges
                                                        #   all     — every node/edge label shown
                                                        _gu_label_opts = ["hover", "colored", "all"]
                                                        _gu_label_mode = _gu_header_cols[1].selectbox(
                                                            "Labels",
                                                            _gu_label_opts,
                                                            index=_gu_label_opts.index(
                                                                st.session_state.get("gu_label_mode", "hover")),
                                                            key=f"gu_label_mode_sel_{_gu_rag_name}_{k}_{k2}",
                                                            help="hover: nothing shown; mouse-over to reveal one. colored: label only coloured nodes/edges. all: every label shown. PNG export follows this setting.",
                                                        )
                                                        st.session_state["gu_label_mode"] = _gu_label_mode
                                                        st.markdown(
                                                        ...
                                                        try:
                                                            _png_bytes = _dgu_main.render_unified_png(_gu_usage, label_mode=_gu_label_mode)
