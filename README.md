...
```json
{
  "SampleConsultingBook": {
    "active": "Y",
    "input": "notion",
    "data_type": "page_index",
    "data_name": "SampleConsultingBook",
    "bucket": "SampleConsultingBook",
    "item_dict": {
      "db": "ConsultingBook_Pages",
      "title": {"名前": "title"},
      "page_id": {"ページID": "rich_text"},
      "summary": {"概要": "rich_text"},
      "tags": {"タグ": "rich_text"},
      "body": {"本文": "rich_text"},
      "category": {"カテゴリ": "select"},
      "sort_order": {"表示順": "number"}
    },
    "chk_dict": {"公開Chk": true},
    "date_dict": {},
    "category_dict": {}
...
「Update RAG Data」を実行すると、`user/common/rag/pages/{data_name}/` 配下に各ページの `.md` ファイルと `_index.json`（ページインデックス）が自動生成されます。

ページデータを手動で配置する場合は、以下の構造で作成します：

```
user/common/rag/pages/SampleConsultingBook/
├── _index.json    # ページ一覧（id, title, summary, tags, category）
├── 1-0.md         # 各ページの本文
├── 1-1.md
└── ...
```

`_index.json` の各ページエントリ：
...
ページデータは `user/common/rag/pages/{DATA_NAME}/` 配下に `_index.json`（ページ一覧）と各ページの `.md` ファイルを配置します。Notion連携で `rags.json` に `"data_type": "page_index"` を定義すると、「Update RAG Data」で自動生成されます。
