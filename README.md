...
- `PERSONA_FILES`: `persona_data/` 配下から読み込むExcelファイル名のリスト（省略時は全xlsx走査）
...
- 不変: `ENGINE` / `SUPPORT_AGENT` / `BOOK` / `SKILL` / `COMMUNICATION`

**並列実行・メモリ制御**:
- WebUIサイドバーで複数ペルソナを選択 → 送信時に `ThreadPoolExecutor`（上限 `MAX_PARALLEL_PERSONAS`）で並列実行
- 並列実行されたseqは自動的に `MEMORY_FLG="N"` がセットされ、次ターンの会話メモリには含まれない
- 「Include Query」チェックボックスONなら、次ターンの**ユーザー入力先頭**に各ペルソナの全文応答を埋め込む（RAGクエリ生成には影響なし）

**サポートエージェント（Phase 6 building blocks）**:
- `agent_50PersonaMerge.json`: 複数ペルソナの応答を公平に統合・要約するサポートエージェント
- プロンプトテンプレ「Persona Merge」内の `{summary_level}` プレースホルダで要約の度合い（`light`/`medium`/`heavy` または自由記述）を指定可能
- `DigiM_Tool.dialog_persona_merge()` ヘルパーから呼び出し
