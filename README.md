...
汎用スケジューラは **Scheduler メニュー**（WebUI上部、Chat / Knowledge Explorer の隣）から管理します。ジョブは `user/common/mst/scheduled_jobs.json` に保存され、Streamlit再起動なしで **Reload Schedulers** ボタンから反映できます。
...
| 散布図オプション | 次元削減（PCA / t-SNE）、Color By、Marker By、Dot Size（Uniform / Newer=Larger） |
| **Search & Plot** | 検索と散布図生成を1ボタンで実行。**散布図は Total＋RAG NAMEごと** を出力。表示順は **散布図 → データ一覧（座標はTotal基準）→ CSV** |
| Clustering | 検索で絞り込まれたデータに対し **Total のみ** でクラスタリング（K-Means / DBSCAN(eps自動推定) / 階層的）。各 RAG NAME には **Total で定義したクラスター割当・色をそのまま適用**（RAG単位で再クラスタリングはしない）。RAG NAMEごとの散布図は横2列で配置 |
...
- Nowaday: 期間要約・基本感情レーダー＋右に「感情／スコア」表・二次感情・継続/新規/減退/変化
...
| 対話セッション | `user/common/analytics/user_memory_explorer/` 配下に専用セッションとして保存 |
