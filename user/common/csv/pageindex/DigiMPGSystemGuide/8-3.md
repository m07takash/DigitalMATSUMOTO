...
**Tools ターゲット**: Thinking Agent が SKILL リストから SKILL 名を選定 → 各 SKILL は `SKILL.TOOLS[name].PHASE` に従い BEFORE / CONTEXT / AFTER のいずれかで自動発火。`HABIT.TOOL_PICK` を経由する旧設計は廃止（下の SKILL 節を参照）。
...
**起動パス（3 段構え、優先順・マージ実行）**:
- **1. Slash command** `/<name> <query>` — 該当 SKILL がヒットしたら `<query>` を HABIT の user_query として使い、SKILL は自分の PHASE で発火
- **2. `SKILL.TOOLS[name].MAGIC_WORDS` マッチ** — ユーザーのクエリに magic word が含まれていれば無条件で発火。HABIT.MAGIC_WORDS と同じ発想でスキル固有の「呼び出し表現」を書く
- **3. Thinking Mode** — Thinking Targets に `Tools` を含めると Thinking Agent が SKILL の `description` を見て選定
- **同時使用可 & dedup** — 3 経路が同じ SKILL を選んでも 1 回だけ実行。順序は slash → magic → thinking
...
| `recall_similar_experience` | BEFORE | Vector KNOWLEDGE から自分の類似経験を想起 | 「似た経験」「類似した経験」「過去に似た」「思い出したい」 |
| `analyze_attachment` | BEFORE | 添付 CSV/TXT/MD の統計特徴抽出 | 「添付を分析」「この資料を分析」「このデータの特徴」 |
| `mood_score` | AFTER | ユーザー & アシスタントの Plutchik 8 感情スコア | 「感情スコア」「今の気持ちを表示」「感情を見せて」 |
| `self_critique` | AFTER | 応答に対する 3 軸のセルフクリティーク | 「自分の回答に反論」「批判的に検討」「セルフクリティーク」 |
| `translate_response` | AFTER | 応答を指定言語 (既定 English) に翻訳 | 「英訳して」「英語に翻訳」「translate to English」 |
| `slide_deck_prompt` | AFTER | 応答を PowerPoint 化するときの構成を **スライド生成 AI 向けの投入プロンプト**として作成 | 「スライドにまとめて」「パワポで」「PowerPointで」「プレゼン資料にまとめる」「スライドAI向け」 |
