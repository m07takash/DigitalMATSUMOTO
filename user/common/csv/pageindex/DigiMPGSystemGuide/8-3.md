...
**起動パス**:
- **Slash command** `/WebSearch <query>` — 該当 SKILL がヒットしたら `<query>` を HABIT の user_query として使い、SKILL は自分の PHASE で発火
- **Thinking Mode** — Thinking Targets に `Tools` を含めると Thinking Agent が SKILL 名を選定
- **両方同時可** — slash 優先で dedup、同じ SKILL が両経路で選ばれても 1 回だけ実行
