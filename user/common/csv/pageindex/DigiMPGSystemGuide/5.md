...
| `DigiM_Session.py` | セッション・チャット履歴管理（保存バックエンドは `DigiM_SessionStore` に委譲） |
...
| `DigiM_MetaSearch.py` | 汎用メタ検索（DATE / CATEGORY / NUMBER / TEXT）の WHERE ビルダ・per-chunk BONUS 積算・`@auto` サイドカー I/O |
