...
KNOWLEDGE / BOOK の各 `DATA` 要素に `META_SEARCH.CONDITIONS[]` を書くと、支援エージェント `META_EXTRACT`（`agent_55MetaExtract.json`）がユーザークエリから各条件の値を **1 コールで一括抽出** し、該当したチャンクに `BONUS` 倍率を乗せて類似度を調整します。`BONUS < 1` は boost（距離が小さくなる方向）、`> 1` は penalty。複数条件がヒットすると倍率が積算されます。旧形式 `"META_SEARCH": {"CONDITION":["DATE"],"BONUS":0.5}` は起動時に自動変換されるため、既存 JSON は無修正で動きます。
