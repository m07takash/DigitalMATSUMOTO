...
> **Thinking と実行既定はエージェント側**: `Thinking Mode` / `Thinking Targets` / `Max Thinking Turns` はユーザー Defaults ではなくエージェント JSON トップレベルの `THINKING` ブロックで管理します。同じ発想で、Conversation Settings 各種チェックボックスの既定値はエージェント JSON の `EXECUTION_DEFAULTS` ブロックにも書けます（Defaults より優先されます）。詳細は「エージェントの設定 → THINKING / EXECUTION_DEFAULTS」ページを参照。
